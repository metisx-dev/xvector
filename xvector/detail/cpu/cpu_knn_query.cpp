...
CpuKnnQuery::CpuKnnQuery(CpuContext* context,
                         xvecKnnType type,
                         const std::shared_ptr<uint8_t[]>& vector,
                         xvecFloatType floatType,
                         std::size_t dimension,
                         std::size_t k)
    : Managed<CpuKnnQuery>(context),
      CpuQuery(CpuQuery::Knn),
      type_(type),
...
template <typename Float>
void createResult(CpuKnnQuery* query)
{
    if (query->result() == nullptr)
    {
        ...
        auto result = CpuKnnResult::create(context, query->floatType(), scores, indices, vectorArrays);
        query->setResult(result);
    }
}

template <typename Float, typename Compare>
void insertResult(CpuKnnQuery* query, Float score, std::size_t index, VectorArray* vectorArray)
{
    Compare compare;
...
void searchByIndexArray(CpuKnnQuery* query)
{
...
template <typename Float, xvecKnnType KnnType>
void searchByVectorArray(CpuKnnQuery* query)
{
    createResult<Float>(query);
...
template <typename Float, xvecKnnType KnnType>
void searchByTargetType(CpuKnnQuery* query)
{
...
template <typename Float>
void searchByType(CpuKnnQuery* query)
{
